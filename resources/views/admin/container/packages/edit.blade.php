...
@section('admin-content')
    <div class="w-full flex items-center justify-between gap-5 mb-5">
        <div>
            <div class="text-2xl font-extrabold text-theme-text-main">Edit Package: {{ $package->name }}</div>
            <div class="text-[13px] text-theme-text-muted mt-1">Update the details of this pricing package.</div>
        </div>
    ...
    <form action="{{ route('admin.packages.update', $package->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
            ...
            <div class="col-span-2 space-y-5">
                <div class="bg-theme-card rounded-[14px] border border-theme-border p-6 shadow-sm space-y-5">
                    <div class="grid grid-cols-2 gap-5">
                        ...
                        <div class="space-y-2">
                            <label class="text-[12px] font-bold text-theme-text-muted uppercase">Display Price</label>
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        <div class="space-y-2">
                            <label class="text-[12px] font-bold text-theme-text-muted uppercase">Numeric Amount (for Payment)</label>
                            <input type="number" step="0.01" name="amount" value="{{ old('amount', $package->amount) }}" required
                                class="w-full bg-theme-bg border border-theme-border rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-theme-primary">
                            @error('amount') <p class="text-red-500 text-[11px]">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="service_id" class="w-full bg-theme-bg border border-theme-border rounded-lg px-4 py-2.5 text-[13px] outline-none focus:border-theme-primary">
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ old('service_id', $package->service_id) == $service->id ? 'selected' : '' }}>{{ $service->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="space-y-5">
                <div class="bg-theme-card rounded-[14px] border border-theme-border p-6 shadow-sm space-y-6">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="popular" id="popular" value="1" {{ $package->popular ? 'checked' : '' }} class="w-4 h-4 rounded border-theme-border text-theme-primary focus:ring-theme-primary">
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-theme-border">
                        <div class="space-y-2">
                            ...
                            <input type="color" name="color" value="{{ $package->color }}" class="w-full h-10 p-1 bg-theme-bg border border-theme-border rounded">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[11px] font-bold text-theme-text-muted uppercase">Text Color</label>
                            <input type="color" name="text_color" value="{{ $package->text_color }}" class="w-full h-10 p-1 bg-theme-bg border border-theme-border rounded">
                        </div>
                    </div>
