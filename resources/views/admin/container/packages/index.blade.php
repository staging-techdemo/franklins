...
@section('admin-content')
    <div class="w-full flex items-center justify-between gap-5 mb-5">
        <div>
            <div class="text-2xl font-extrabold text-theme-text-main">Package Management</div>
            <div class="text-[13px] text-theme-text-muted mt-1">Manage pricing plans and packages for your services.</div>
        </div>
    ...
    <div class="bg-theme-card text-theme-text-main rounded-[14px] border border-theme-border overflow-hidden shadow-sm">
        <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between">
            <h3 class="text-[15px] font-extrabold text-theme-text-main">All Packages</h3>
            <form action="{{ route('admin.packages.index') }}" method="GET" class="flex items-center gap-3">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-theme-bg border-b border-theme-border text-theme-text-main">
                    <tr>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Name</th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Price</th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Linked Service</th>
                ...
                <tbody class="divide-y divide-theme-border">
                    @forelse ($packages as $package)
                        <tr class="hover:bg-theme-hover transition-colors">
                            <td class="px-6 py-4">
                                <div class="text-[13.5px] font-bold text-theme-text-main">{{ $package->name }}</div>
                                @if($package->popular)
                                    <span class="text-[9px] font-black uppercase text-amber-500 tracking-tighter">Popular</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-main">{{ $package->price }} <span class="text-gray-400 text-[11px]">{{ $package->duration }}</span></td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-muted">
                                {{ $package->service ? $package->service->title : 'Global' }}
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-0.5 rounded-full text-[10.5px] font-bold {{ $package->status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-theme-text-muted">No packages found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
