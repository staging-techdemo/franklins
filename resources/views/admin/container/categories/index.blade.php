...
@section('admin-content')
    <div class="w-full flex items-center justify-between gap-5 mb-5">
        <div>
            <div class="text-2xl font-extrabold text-theme-text-main">Category Management</div>
            <div class="text-[13px] text-theme-text-muted mt-1">Manage categories for blogs and services.</div>
        </div>
    ...
    <div class="bg-theme-card text-theme-text-main rounded-[14px] border border-theme-border overflow-hidden shadow-sm">
        <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between">
            <h3 class="text-[15px] font-extrabold text-theme-text-main">All Categories</h3>
            <form action="{{ route('admin.categories.index') }}" method="GET" class="flex items-center gap-3">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-theme-primary text-white rounded-[8px] text-[12px] font-bold">Search</button>
            </form>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-theme-bg border-b border-theme-border text-theme-text-main">
                    <tr>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Name
                        </th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Slug
                        </th>
                ...
                <tbody class="divide-y divide-theme-border">
                    @forelse ($categories as $category)
                        <tr class="hover:bg-theme-hover transition-colors">
                            <td class="px-6 py-4 text-[13.5px] font-bold text-theme-text-main">{{ $category->name }}</td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-muted">{{ $category->slug }}</td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-0.5 rounded-full bg-theme-primary-light text-theme-primary text-[10.5px] font-bold uppercase tracking-wider">
                                    {{ $category->type }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-theme-text-muted">No categories found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
