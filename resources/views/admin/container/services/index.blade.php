...
@section('admin-content')
    <div class="w-full flex items-center justify-between gap-5 mb-5">
        <div>
            <div class="text-2xl font-extrabold text-theme-text-main">Service Management</div>
            <div class="text-[13px] text-theme-text-muted mt-1">Manage care services, descriptions, and inclusion lists.</div>
        </div>
    ...
    <div class="bg-theme-card text-theme-text-main rounded-[14px] border border-theme-border overflow-hidden shadow-sm">
        <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between">
            <h3 class="text-[15px] font-extrabold text-theme-text-main">All Services</h3>
            <form action="{{ route('admin.services.index') }}" method="GET" class="flex items-center gap-3">
            ...
            <table class="w-full text-left">
                <thead class="bg-theme-bg border-b border-theme-border text-theme-text-main">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Slug</th>
                ...
                <tbody class="divide-y divide-theme-border">
                        ...
                        <tr class="hover:bg-theme-hover transition-colors">
                            <td class="px-6 py-4">
                                <img src="{{ $service->image ? asset($service->image) : asset('assets/service-detail.jpg') }}" 
                                     alt="{{ $service->title }}" class="w-12 h-12 rounded-lg object-cover">
                            </td>
                            <td class="px-6 py-4 text-[13.5px] font-bold text-theme-text-main">{{ $service->title }}</td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-muted">{{ $service->slug }}</td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-0.5 rounded-full text-[10.5px] font-bold {{ $service->status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-theme-text-muted">No services found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
