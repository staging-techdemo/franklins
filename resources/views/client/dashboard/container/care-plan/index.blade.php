...
<div class="space-y-6">
        ...
        <div class="bg-theme-card border border-theme-border rounded-[14px] shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between bg-theme-hover/50">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-full bg-theme-primary text-white flex items-center justify-center font-bold">
                        {{ strtoupper(substr($booking->service->title, 0, 1)) }}
                    </div>
                    <div>
                        <h3 class="text-[15px] font-extrabold text-theme-main">{{ $booking->service->title }}</h3>
                        <p class="text-[11px] text-theme-muted uppercase tracking-widest font-bold">{{ $booking->plan_type }} Plan</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="px-3 py-1 rounded-full bg-green-100 text-green-700 text-[10px] font-black uppercase">Active</span>
                </div>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    ...
                    <p class="text-[12px] text-theme-muted mt-1">{{ $booking->patient_age }} years · {{ $booking->relationship }}</p>
                </div>
                <div>
                    ...
                    <p class="text-[12px] text-theme-muted mt-1">Starts: {{ $booking->preferred_date ? \Carbon\Carbon::parse($booking->preferred_date)->format('d M Y') : 'Immediate' }}</p>
                </div>
                <div>
            ...
            @if($booking->notes)
                <div class="px-6 pb-6 pt-0">
                    <div class="p-4 bg-theme-bg rounded-[10px] border border-theme-border italic text-[12.5px] text-theme-muted">
                        "{{ $booking->notes }}"
                    </div>
                </div>
            @endif
    ...
    @empty
        <div class="bg-theme-card border border-theme-border rounded-[14px] p-12 text-center shadow-sm">
            ...
            <p class="text-theme-muted text-[14px] max-w-md mx-auto mb-8">You haven't booked any care services yet. Explore our packages to get started with professional home care.</p>
            <a href="{{ route('packages') }}" class="px-8 py-3 bg-theme-primary text-white rounded-[12px] font-bold shadow-lg hover:bg-theme-primary-hover transition-all">Browse Packages</a>
        </div>
    @endforelse
</div>
@endsection
