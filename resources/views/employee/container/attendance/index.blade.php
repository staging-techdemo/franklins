...
<div class="grid sm:grid-cols-1 xm:grid-cols-1 grid-cols-4 gap-5 my-5">
    <div class="bg-theme-card rounded-[14px] p-5 border border-theme-border shadow-sm">
        ...
        <div class="text-2xl font-extrabold text-theme-text-main">{{ $presentToday ?? 0 }}</div>
    ...
    <div class="bg-theme-card rounded-[14px] p-5 border border-theme-border shadow-sm">
        ...
        <div class="text-2xl font-extrabold text-theme-text-main">{{ $lateToday ?? 0 }}</div>
        <div class="mt-2 flex items-center">
    ...
    <div class="bg-theme-card rounded-[14px] p-5 border border-theme-border shadow-sm">
        ...
        <div class="text-2xl font-extrabold text-theme-text-main">{{ $absentToday ?? 0 }}</div>
        <div class="mt-2 flex items-center">
    ...
    <div class="bg-theme-card rounded-[14px] p-5 border border-theme-border shadow-sm">
        ...
        <div class="text-2xl font-extrabold text-theme-text-main">{{ $onLeaveCount ?? 0 }}</div>
        <div class="mt-2 flex items-center">
            <span
    ...
    <div class="lg:col-span-2">
        <div class="bg-theme-card rounded-[14px] border border-theme-border overflow-hidden shadow-sm">
            <div class="px-6 py-5 border-b border-theme-border flex items-center justify-between">
                <h3 class="text-[15px] font-extrabold text-theme-text-main">Today's Attendance — {{
                    \Carbon\Carbon::now()->format('M d, Y') }}</h3>
                <select
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-theme-border">
                        ...
                        <tr class="hover:bg-theme-bg transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-[13px] font-bold text-theme-text-main">{{ auth()->user()->name }}
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-main">{{
                                \Carbon\Carbon::parse($a->check_in)->format('h:i A') }}</td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-main">{{ $a->check_out ?
                                \Carbon\Carbon::parse($a->check_out)->format('h:i A') : '—' }}</td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-main font-bold">{{ $a->check_out ?
                                number_format(\Carbon\Carbon::parse($a->check_out)->diffInMinutes(\Carbon\Carbon::parse($a->check_in))/60,1)
                                : '—' }} hrs</td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-0.5 rounded-full {{ $a->status === 'Present' ? 'bg-green-100 text-green-600' : ($a->status === 'Late' ? 'bg-amber-100 text-amber-600' : ($a->status === 'On Leave' ? 'bg-theme-hover text-theme-text-main' : 'bg-red-100 text-red-600')) }} text-[10.5px] font-bold">{{
                                    $a->status }}</span>
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-theme-text-muted">No attendance records
                                found for this month.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
