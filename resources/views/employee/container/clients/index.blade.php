...
@section('employee-content')
    <div class="w-full flex items-center justify-between gap-5 mb-5">
        <div>
            ...
            <div class="text-[13px] text-theme-text-muted mt-1">Manage profiles and active assignments for clients under your care.</div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-theme-bg border-b border-theme-border text-theme-text-main">
                    <tr>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">ID</th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Client Name</th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Region</th>
                        <th class="px-6 py-3 text-[10.5px] font-bold text-theme-text-muted uppercase tracking-widest">Care Plan</th>
                ...
                <tbody class="divide-y divide-theme-border">
                        ...
                        <tr class="hover:bg-theme-hover transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-[13.5px] font-bold text-theme-text-main">{{ $client->user->name }}</div>
                                        <div class="text-[11px] text-theme-text-muted">Contact: {{ $client->phone ?? $client->user->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-[13px] text-theme-text-main">{{ $client->region ?? 'N/A' }}</td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-0.5 rounded bg-theme-primary-light text-theme-primary text-[11px] font-bold">{{ $client->care_plan ?? 'N/A' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-theme-text-muted">
